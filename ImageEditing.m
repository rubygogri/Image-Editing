...
function fname =ImageEditing(hObject,eventdata)
...
figure;imagesc(Ang),colormap(winter(50)),axis equal , axis tight;
...
for k = 1:N
    % Boundary k is the parent of a hole if the k-th column
    % of the adjacency matrix A contains a non-zero element
    if (nnz(A(:,k)) > 0)     %Number of non-zero elements
        plot(boundary(:,2),...
